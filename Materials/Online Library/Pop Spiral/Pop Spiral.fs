/*{
    ...
    "DESCRIPTION": "From https://www.shadertoy.com/view/XtsBDf",
    ...
    "INPUTS": [
        ...
        {
            "LABEL": "Color Fade/Level",
            "NAME": "mat_color_level",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
...
float mat_a1_time = (mat_a1_time_source - mat_a1_offset * 8. * mat_a1_offset_scale) * 0.5;
float mat_a2_time = (mat_a2_time_source - mat_a2_offset * 8. * mat_a2_offset_scale) * 0.5;
float mat_a3_time = (mat_a3_time_source - mat_a3_offset * 8. * mat_a3_offset_scale) * 0.5;
...
float de(vec3 p) {
    p.y += cos(mat_a2_time*2.) * .2 * mat_spin_wobble;
    ...
    p.xy *= r2d(mat_a2_time + p.z);

    vec3 r;
    float d = 0., s = 1.6;

    for (int i = 0; i < mat_iterations; i++)
        r = max(r = abs(mod(p*s + 1., 2. * mat_cycle2) - 1.), r.yzx),
        d = max(d, (.8*mat_limit - min(r.x, min(r.y, r.z))) / s),
        s = sqrt(s) + 7. * mat_cycle1;
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    vec3 ro = vec3(mat_travel_wobble*.1*cos(mat_a1_time), 0, -mat_a1_time), p;
    vec3 rd = normalize(vec3(uv, -1));
    p = ro;
    float it = 0.;
    for (float i=0.; i < 1.; i += .01 * mat_step) {
        it = i;
        float d = de(p);
        if (d < .001) break;
        p += rd * d*.5;
    }
    it /= .9 * sqrt(abs(tan(mat_a3_time*1.3) + p.x*p.x + p.y*p.y)) * mat_color_level;
    vec3 c = mix(vec3(1., 1., 0), vec3(.9, .2, .6)*mat_color_gain, it*pow(sin(p.z), 1./10.));
