/*{
    ...
    "CREDIT": "ilvbrs, adapted by Jason Beyers",

    "DESCRIPTION": "From https://www.shadertoy.com/view/Dll3Rj",
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    // out_color = mod(length(p)-k*(atan(p.y,p.x)+P+mat_time),k*P)/k/P*vec4(.5,.9,.9,1);
    out_color = mod(length(p) - float(mat_count)*k*(atan(mat_distort_y*p.y,mat_distort_x*p.x)+P+mat_time),k*P)/k/P * vec4(1.);
