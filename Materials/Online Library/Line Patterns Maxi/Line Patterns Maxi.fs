/*{
  ...
  "INPUTS": [
    ...
    { "LABEL": "Global/Repeat", "NAME": "mat_repeat", "TYPE": "int", "DEFAULT": 1, "MIN": 1, "MAX": 256 },
    ...
    { "LABEL": "Auto Move/Repeat", "NAME": "mat_automoverepeat", "TYPE": "int", "DEFAULT": 1, "MIN": 1, "MAX": 16 },
    ...
    { "LABEL": "Auto Light/Width", "NAME": "mat_autolightwidth", "TYPE": "float", "DEFAULT": 1.0, "MIN": 0.0, "MAX": 1.0 },
    ...
    { "LABEL": "Auto Light/Repeat", "NAME": "mat_autolightrepeat", "TYPE": "int", "DEFAULT": 1, "MIN": 1, "MAX": 16 },

    { "LABEL": "Noise/Active", "NAME": "mat_noiseactive", "TYPE": "bool", "DEFAULT": false, "FLAGS": "button" },
    { "LABEL": "Noise/Level", "NAME": "mat_noiselevel", "TYPE": "float", "DEFAULT": 1, "MIN": 0.0, "MAX": 1.0 },
    { "LABEL": "Noise/Scale", "NAME": "mat_noisescale", "TYPE": "float", "DEFAULT": 1, "MIN": 0.000001, "MAX": 4.0 },
    { "LABEL": "Noise/Speed", "NAME": "mat_noisespeed", "TYPE": "float", "DEFAULT": 1.0, "MIN": 0.0, "MAX": 3.0 },
    { "LABEL": "Noise/Strob", "NAME": "mat_noisestrob", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 1.0 },
    { "LABEL": "Noise/Shape", "NAME": "mat_noiseshape", "TYPE": "long", "VALUES": ["Sin","Linear","Noise"], "DEFAULT": "Sin" },
    ...
    { "LABEL": "Color/Brightness", "NAME": "mat_brightness", "TYPE": "float", "MIN": -5.0, "MAX": 1.0, "DEFAULT": 0.0 },
    { "LABEL": "Color/Contrast", "NAME": "mat_contrast", "TYPE": "float", "MIN": 1.0, "MAX": 10.0, "DEFAULT": 1 },
  ],
  "GENERATORS": [
    {
    ...
    },
    {
      ...
      "TYPE": "time_base",
      "PARAMS": {"speed": "mat_noisespeed","strob": "mat_noisestrob", "bpm_sync": "mat_bpmsync", "speed_curve":2, "link_speed_to_global_bpm":true }
    }
  ]
}*/
...
vec4 materialColorForPixel(vec2 texCoord)
{
	...
	if (mat_noiseactive) {
		if (mat_noiseshape == 0) { // Sin
			p.x += mat_noiselevel * sin((p.y * mat_noisescale * mat_noisescale + mat_noise_position) * 2 * 3.141592654);
		} else if (mat_noiseshape == 1) { // Line
			float value = mod(p.y * mat_noisescale * mat_noisescale + mat_noise_position,2);
			if (value > 1) {
				value = 2 - value;
			}
			p.x += mat_noiselevel * (-0.5 + value);
		} else { // Noise
	        float value = flowNoise(vec2(mat_noise_position+normalizedCellId,11.34 * p.y * mat_noisescale * mat_noisescale), 0);
			p.x += mat_noiselevel * (-0.5 + value);
		}
	}
    ...
    float halfFinalWidth = mat_width*mat_width * mat_widthValue / 2;
    ...
    if (mat_autolightactive) {
      // Light
        ...
        lightValue = 1 - mat_autolightsize * (1+sin((mat_light_position+normalizedCellId*mat_autolightoffset*mat_autolightrepeat)*2*M_PI))/2;
      #elif defined(mat_autolightshape_IS_In)
        lightValue = 1 - mat_autolightsize * (mod((mat_light_position+normalizedCellId*mat_autolightoffset*mat_autolightrepeat),1));
      #elif defined(mat_autolightshape_IS_Linear)
        lightValue = 1 - mat_autolightsize * abs(mod((mat_light_position+normalizedCellId*mat_autolightoffset*mat_autolightrepeat)*2+1,2)-1);
      #elif defined(mat_autolightshape_IS_Cut)
        lightValue = 1 - mat_autolightsize * step(0.5,mod((mat_light_position+normalizedCellId*mat_autolightoffset*mat_autolightrepeat),1));
      #elif defined(mat_autolightshape_IS_Noise)
        lightValue = 1 - mat_autolightsize * vnoise(vec2((mat_light_position+normalizedCellId*mat_autolightoffset*10*mat_autolightrepeat),2));
      #endif
	  lightValue -= 1-mat_autolightwidth;
	  lightValue *= 1/mat_autolightwidth;
    } else {
      lightValue = 1;
    }
