/*{
    ...
    "DESCRIPTION": "From https://www.shadertoy.com/view/7slfD8",
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    for(float i=0.,s,e,g=0.,t=mat_time;i++<float(mat_iterations1);){
        p=g*d;
        p.z-=.5;
        p=R(p,H(t*.01),t*.2);
        s=1. * mat_fog;
        for(int j=0;j++<mat_iterations2;)
           p=p.x<p.y?p.zxy:p.zyx,
           s*=e=max(1./dot(p,p),1.5 * mat_expand),
           p=abs(p)*e-vec3(2 + mod.x,1.7 + mod.y,.8) * mat_fov;
        g+=e=abs(p.x)/s+1e-4;
        c+=mix(vec3(1),H(log(s)*.4),.4 * mat_colorize)*.015/exp(.1*i*i*e);
    }
    c*=c*c;
    out_color=vec4(c,1);
