/*{
    ...
    "DESCRIPTION": "From https://www.shadertoy.com/view/wdVSDy",
...
#define hash1( n ) fract(sin(n)*43758.5453 + mat_cycle * 2.)
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    float border1 = 0.025 * pow(mat_border1,2.);
    float border2 = 0.005 * mat_border2;
    ...
    float a = atan(st.x,st.y)/PI
        ...
        ,c2 = (a*1.5+2.);
    c2 += (c2 < 1.) ? 3. : 0.;
    float l = dot(abs(st),normalize(vec2(1. * mat_iso_x,1.73 * mat_iso_y)));
    l = log2(max(l,abs(st.x)))-iT;
    vec2 c_gr = (mod(l,2.)>1.) ? vec2(c1,c2) : vec2(c2,c1) ;

    c_gr *= mat_angle;
    float sm2 = fwidth(l);
    float i = step(.5,hash1(floor(l)*floor(c_gr.x)) / mat_fill);
    float c = i > 0.5
            ? 0.
