/*{
    ...
    "DESCRIPTION": "Hyperbolic Poincare double sided Truchet weave, from https://www.shadertoy.com/view/tljyRR",
    ...
    "INPUTS": [
        ...
        {
            "LABEL": "Shape/Ring Pattern",
            "NAME": "mat_ring_pattern",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
        ...
        {
            "LABEL": "Shape/Ring Shadow",
            "NAME": "mat_ring_shadow",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
...
/*
    ...
    which is well worth the look. STB's "Hyperbolic PoincarÃƒÂ© transformed"
    ...
    //
    Hyperbolic PoincarÃƒÂ© transformed - stb
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    float shape = Polygon(p, float(N), .8 * mat_pattern2);
    ...
    col = mix(col, oCol, 1. - smoothstep(0., sf, shape + .04 * mat_shadow2));//.35*f*f
    // Vertices.
    ...
    for(int i = 0; i<N; i++){
        ...
        // the Bezier curves.
        vec2 mpi = mix(vp[i], vp[(i + 1)%N], .5 * mat_pattern1);
    ...
    // this is the FisherÃ¢â‚¬â€œYates method, but don't quote me on it. It's been a while since
    ...
    for(int i = 0; i<N; i++){
        ...
        ln[i] = doSeg(p, mp[j], mp[jp1], lnOff) - .105 * pow(mat_thickness,0.75);
    ...
    col = mix(col, vec3(0), ringBlend*(1. - smoothstep(0., .5, pat2))*.7 * mat_shading2);
     ...
     for(int i = 0; i<N; i++){
         ...
         pat2 = mix(1., 0., ringBlend*(1. - smoothstep(0., .5, pat2))*.7 * mat_lines);

         // Bezier distance field layers.
         vec3 bgC = col;
         col = mix(col, vec3(0), (1. - smoothstep(0., sf*5., ln[i]))*.3 * mat_shadow1);
    ...
    float ring = abs(lu - 1.)/mat_ring - .05; // Ring distance field.

    // Dial pattern.
    float ang = atan(uv.x, uv.y)/TAU;
    float pat = abs(fract(ang*72.) - .5)*2. * mat_ring_pattern;
    pat = smoothstep(0., sf, pat - .125);

    // Rendering the drop shadow, etc.

    col = mix(col, vec3(0), (1. - smoothstep(0., sf*5., ring-.01))*.6 * mat_ring_shadow);
    ...
    col = mix(col, min( mix(col, vec3(1)*dot(col, vec3(.299, .587, .114) * mat_shading3),
