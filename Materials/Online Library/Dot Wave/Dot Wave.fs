/*{
    ...
    "CREDIT": "Norihiro Narayama, adapted by Jason Beyers",

    "DESCRIPTION": "From https://neort.io/art/bradq9s3p9f04urh8ov0?index=656&origin=tag",
    ...
    "INPUTS": [
        ...
        {
            "LABEL": "Color/Color 1 Gain",
            "NAME": "mat_color1_glow",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
        ...
        {
            "LABEL": "Color/Color 2 Gain",
            "NAME": "mat_color2_glow",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
        ...
        {
            "LABEL": "Color/Color 3 Gain",
            "NAME": "mat_color3_glow",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
        ...
        {
            "LABEL": "Duotone/Enable",
            "NAME": "mat_enable_duotone",
            "TYPE": "bool",
            "DEFAULT": false,
            "FLAGS": "button"
        },
        {
            "LABEL": "Duotone/Front Color",
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    vec3 color = mat_color3.rgb * mat_color3_glow;
    vec2 st2 = st;
    st2 = mod(st2 * 30.0 * mat_dot_scale, 1.0);
    st2 -= 0.5;

    st2 /= mat_size;

    st *= 2.0;
    float lng = length(st) * mat_shape_scale;
    float at = atan(st.y, st.x) * mat_shape_rot;

    color *= fract(abs(dot(sin(at) * lng,sin(at) * lng)) + mat_time * 0.5);
    color *= fract(abs(dot(cos(at) * lng,cos(at) * lng)) - mat_time * 0.5);
    color = mix(color, mat_color2.rgb * mat_color2_glow, 0.5);
    color = mix(color, mat_color1.rgb * mat_color1_glow, fract(abs(dot(sin(at) * lng,sin(at) * lng)) - mat_time));
    color -= length(st2);

    out_color = vec4(color * 3.0, 1.0);
