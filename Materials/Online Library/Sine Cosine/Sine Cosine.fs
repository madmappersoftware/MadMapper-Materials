/*{
    ...
    "CREDIT": "opexu, adapted by Jason Beyers",

    "DESCRIPTION": "From https://www.shadertoy.com/view/7dtBDB",
    ...
    "INPUTS": [
        ...
        {
            "LABEL": "Pattern/Noise Cycle",
            "NAME": "mat_noise_cycle",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    float s = sin((uv.x-mat_time*.01)*25.)*cos((uv.y+mat_time*.02)*25.)*.5+.5 * mat_noise_cycle;

    s *= mat_noise_level;
    uv += sin(s)*cos(s)+atan(s-uv.x);
    uv = fract(uv+cos(mat_time*.3+sin(uv.y))*sin(mat_time*.2+cos(uv.x)));
    vec3 col = vec3(uv.x+sin(mat_time)*.1+.1, uv.y+cos(mat_time)*.1, uv.y+cos(mat_time*2.3)*.1 * mat_color_cycle1+.1 * mat_color_cycle2);
    // Output to screen
    ...
    if (mat_luma_to_alpha && mat_luma_mode == 0) {
        out_color.a = mat_luma(out_color * mat_luma_sensitivity * 4.) - mat_luma_threshold * 4. - 1.;
    }
