/*{
  ...
  "INPUTS": [
    ...
    { "LABEL": "Global/Count", "NAME": "mat_line_count", "TYPE": "int", "DEFAULT": 1, "MIN": 1, "MAX": 16 },
    { "LABEL": "Global/H Repeat", "NAME": "mat_repeat", "TYPE": "float", "DEFAULT": 1, "MIN": 0.01, "MAX": 16.0 },
...
mat3 makeTransformMatrix(float normalizedCellId)
{
    ...
    if (mat_automoveactive) {
        ...
        translateX = mat_automovesize * sin((mat_move_position+normalizedCellId*mat_automoveoffset/mat_repeat)*2*M_PI) / 2;
      #elif defined(mat_automoveshape_IS_In)
        translateX = mat_automovesize * (0.5-mod((mat_move_position/mat_repeat+normalizedCellId*mat_automoveoffset/mat_repeat),1/mat_repeat));
      #elif defined(mat_automoveshape_IS_Linear)
        translateX = mat_automovesize * (0.5-abs(mod((mat_move_position/mat_repeat+normalizedCellId*mat_automoveoffset/mat_repeat)*2+1,2)-1));
        ...
        translateX = mat_automovesize * (0.5-vnoise(vec2((mat_move_position/mat_repeat+normalizedCellId*mat_automoveoffset*10/mat_repeat),0)));
      #elif defined(mat_automoveshape_IS_Smooth_In)
        translateX = mat_automovesize * (-0.5 * sin(-M_PI/2 + mod((mat_move_position/mat_repeat+normalizedCellId*mat_automoveoffset/mat_repeat),1/mat_repeat)*M_PI));
      #endif
    }
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    float baseNormalizedCellId = cellId / float(mat_cells_x*mat_cells_y);
	float value = 0;

	for (int lineNumber=0; lineNumber<mat_line_count; lineNumber++) {
	    float mat_widthValue;
		float normalizedCellId = fract(baseNormalizedCellId + lineNumber/float(mat_line_count));
