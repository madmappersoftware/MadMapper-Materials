/*{
    ...
    "CREDIT": "hoeled, adapted by Jason Beyers",

    "DESCRIPTION": "From https://www.shadertoy.com/view/tsyBRy",
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    float aa = 1.5 / (1000. * mat_aa);

    // deformation
    uv *= 2.;
    uv *= Rot(mat_time*.09);
    uv.x += atan(0.07, uv.y*uv.y*(sin(mat_time*.5)*.2+.2))*.4 * mat_deform_1;
    uv /= atan(4., mat_time*4.)+2. * mat_deform_2;
    uv *= Rot(mat_time*.2);
    uv.x *= uv.x;
