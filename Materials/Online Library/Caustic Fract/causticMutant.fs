/*{
    ...
    "TAGS": "noise",
    "INPUTS": [
        { "LABEL": "Speed", "NAME": "speed", "TYPE": "float", "MIN" : 0.0, "MAX" : 1.0, "DEFAULT": 0.05 },
        { "LABEL": "Reverse", "NAME": "reverse", "TYPE": "bool", "DEFAULT": 0, "FLAGS": "button" },
        { "LABEL": "Scale", "NAME": "scale", "TYPE": "float", "DEFAULT": 0.075, "MIN": 0.01, "MAX": 0.250 },
        { "LABEL": "Brightness", "NAME": "brightness", "TYPE": "float", "MIN": -4, "MAX": 0.5, "DEFAULT": -1.75 },
        { "LABEL": "Contrast", "NAME": "contrast", "TYPE": "float", "DEFAULT": 0.50, "MIN": -1.0, "MAX": 1.0 }
    ],
    "GENERATORS": [
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    for (int n = 0; n < MAX_ITER; n++)
    {
        ...
        i = ( cos(w+p)+p)+ vec2(fract(t - i.x) + sin(t + i.y),  //
        sin(t - i.y) + cos(t + i.x));
    
        c += 1.0/length(vec2(p.x / (fract(i.x+t)/inten),
        p.y / (cos(i.y+t)/inten)));
    }
