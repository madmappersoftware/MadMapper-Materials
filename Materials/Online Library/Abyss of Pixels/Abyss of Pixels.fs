...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    vec2 p = uv;
    float v = 0.;
    for(float i = 0.; i < int(mat_layers);i++){
        float L = 1. + i - fract(mat_time);

        vec2 q = p / atan(.005, L) / 500.;
        L = dot(q, q) * 3e2 + L * L;
        L *= mat_grad_level;
        float n = hash(ceil(mat_time) + i) * PI2;
        q = ceil(q * 20. + vec2(cos(n) * pow(mat_offset_x, 2.), sin(n) * pow(mat_offset_y, 2.))) / 20.;
        if(v == 0.){
            v += step(1. * 1./mat_fill_2, hash2D(q + n * .01) * mat_fill_3 + dot(q, q) * 2. * mat_fill_1) * exp(-L * .01 / mat_grad_size) * mat_gain;
        }
    }

    out_color = vec4(vec3(v), 1.);


    out_color.rgb *= mat_front_color.rgb;
