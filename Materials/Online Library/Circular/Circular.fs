/*{
    "CREDIT": "Joe Griffith",
    "DESCRIPTION": "Circular noise array",
    "TAGS": "noise,circle",
    "VSN": "1.0",
    "INPUTS": [ 
		{ "LABEL": "Speed", "NAME": "mat_speed", "TYPE": "float", "MIN": 0.0, "MAX": 2.0, "DEFAULT": 1.0 }, 
		{ "LABEL": "Scale", "NAME": "mat_scale", "TYPE": "float", "MIN": 1.0, "MAX": 6.0, "DEFAULT": 6.0 }, 
       	{ "LABEL": "Noise Amp", "NAME": "mat_noise_amp", "TYPE": "float", "MIN": 0.0, "MAX": 1.0, "DEFAULT": 0.05 },		
...
vec4 materialColorForPixel( vec2 texCoord )
{
	...
	// use MadNoise libray
		float K = worleyNoise(vec3(uv,mat_animation_time));
		K *= (mat_noise_amp*50+1.5);

	
	float positionX=0;	
	float positionY=K+.20;
	float width = mat_scale;	
	float height = K;	
	
	vec3 color = HardRectangle(vec2(positionX,positionY),width,height);
	color += HardRectangle(vec2(positionX,K+1),width,height);
	color -= HardRectangle(vec2(positionX,K+.5),width,height);
	color += HardRectangle(vec2(positionX,K+3.0),width,height);
	color += HardRectangle(vec2(positionX,K-.5),width,height);
