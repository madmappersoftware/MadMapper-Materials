...
vec4 materialColorForPixel( vec2 texCoord ) {
    ...
    p *= mat_zoom;
    float tun=5./sqrt(dot(p, p)),en=float(fract(7.7/tun+iTime)>=.07/fract(tun+sin(iTime))),l=float(.275<fract(tun+sin(iTime*1.)+atan(p.x, p.y)*7.7));
    return vec4(tun<.05||en<l);
}