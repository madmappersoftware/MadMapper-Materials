...
{"LABEL": "Speed Y", "NAME": "mat_speedy", "TYPE": "float", "MIN": 0.0, "MAX": 10.0, "DEFAULT": 1.0 }, 
{"LABEL": "Zoom", "NAME": "mat_density", "TYPE": "float", "MIN": 0.0, "MAX": 2.0, "DEFAULT": 1.0 }, 
	{ "LABEL": "Direction", "NAME": "mat_direction", "TYPE": "point2D", "MAX": [ 1.0, 1.0 ], "MIN": [ -1.0, -1.0 ], "DEFAULT": [ 1.0, -1.0 ] },
    ],
	"GENERATORS": [
...
vec4 materialColorForPixel( vec2 texCoord )
{
		...
		float speed=2.0;
				...
				vec2 GA = vec2(0.);
				GA.x-=mat_timex*mat_direction.x;
				GA.y+=mat_timey*mat_direction.y;
				...
				F1 = 1.0-cellular2x2((uv+(GA*0.1))*8.0*mat_density);	
				A1 = 1.0-(A*1.0);
				N1 = smoothstep(0.998,1.0,F1)*1.0*A1;	

				F2 = 1.0-cellular2x2((uv+(GA*0.2))*6.0*mat_density);	
				A2 = 1.0-(A*0.8);
				N2 = smoothstep(0.995,1.0,F2)*0.85*A2;				

				F3 = 1.0-cellular2x2((uv+(GA*0.4))*4.0*mat_density);	
				A3 = 1.0-(A*0.6);
				N3 = smoothstep(0.99,1.0,F3)*0.65*A3;				

				F4 = 1.0-cellular2x2((uv+(GA*0.6))*3.0*mat_density);	
				A4 = 1.0-(A*1.0);
				N4 = smoothstep(0.98,1.0,F4)*0.4*A4;				

				F5 = 1.0-cellular2x2((uv+(GA))*1.2*mat_density);	
				A5 = 1.0-(A*1.0);
				N5 = smoothstep(0.98,1.0,F5)*0.25*A5;				
								
				float Snowout=N5+N4+N3+N2+N1;
				
				vec4 col = vec4(Snowout*0.9, Snowout, Snowout*1.1, 1.0);
