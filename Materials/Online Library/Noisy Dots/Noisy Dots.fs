/*{
    "CREDIT": "sleeping, adapted by Jason Beyers",

    "DESCRIPTION": "From https://www.shadertoy.com/view/DlXSWH",
...
float sdf(vec2 uv){
    uv = _map(uv, vec2(-.5 - .3*cos(.5*mat_time), .4 + .3*sin(.7*mat_time)), 5);

    float c = .08 / mat_noise_scale;
    vec2 q = mod(uv + .5*c, c) - .5*c; // repetition of shape
    vec2 m = mod(uv + .5*c, 2.*c) - c;
    float v = dot(q,q) - c*c*.13 * pow(mat_mix * 2., 2.5);
    return mix(
...
vec3 _colorize(float d){
    return sin(d + .45 * mat_colorize2)*vec3(
        .6 + .3*d,
        .5 - .3*d,
        .9 - .3*d
    );
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    float d = sdf(uv);
    vec3 col = _colorize(d) * mat_colorize - _noise(uv)*.1 * mat_grain;
