/*{
    ...
    "DESCRIPTION": "From https://www.shadertoy.com/view/fdSGzt",
    ...
    "INPUTS": [
        ...
        {
            "LABEL": "Fractal/Color Scale",
            "NAME": "mat_color_scale",
            "TYPE": "float",
            "MIN": 0.0,
            "MAX": 4.0,
            "DEFAULT": 1.0
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    for(float i=0.,s,e,g=1.5 * mat_cutoff;
        ++i<float(mat_iterations1);
        out_color.xyz+=mat_glow*.1*mix(vec3(1),H(log(s)*.3 * mat_color_scale),.8)*exp(-12.*i*i*e)
    )
    {
        p=g*d-vec3(-.2,.3,2.5) * mat_fov;

        vec3 rot_pos = vec3(1,2.*sin(mat_time*.1),3);
        rot_pos.x += orbit.x;
        rot_pos.y += orbit.y;

        p=R(p,normalize(rot_pos),mat_time*.2);
        s=5. * mat_cycle2;
        p=p/dot(p,p)+1. * mat_cycle1;
        for(int i=0;i++<mat_iterations2;p*=e)
            p=1.-abs(p-1.),
