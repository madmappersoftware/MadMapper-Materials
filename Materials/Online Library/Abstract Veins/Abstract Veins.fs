/*{
    "CREDIT": "Alkama, adapted by Jason Beyers",

    "DESCRIPTION": "From https:\/\/www.shadertoy.com\/view\/tdsGR8",
...
vec4 materialColorForPixel(vec2 texCoord)
{
    ...
    vec2 pa = uv + .1*vec2(sin(sin(mat_time)*2.),cos(sin(mat_time*.8)+mat_time));
    vec2 pb = uv + mat_cycle1*.6*vec2(sin(mat_time*.9),cos(sin(mat_time*.2)*2.9*mat_cycle2));
    float d = fract(length(pa)*10.*mat_shape_scale) * 5.*fract(length(pb)*2.);

    d = fract(d);
    float w = pow(mat_fill, 0.25)*.7+.2*sin(mat_time*2.);
    float a = 0.1;
