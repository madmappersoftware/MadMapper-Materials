/*{
  ...
  "INPUTS": [
    ...
    { "LABEL": "Global/Distort", "NAME": "mat_distort", "TYPE": "float", "DEFAULT": 0.1, "MIN": 0.0, "MAX": 1.0 },
	...
	{ "LABEL": "Global/One Shot", "NAME": "mat_oneshot", "TYPE": "bool", "DEFAULT": false, "FLAGS": "button" },
...
float noiseSin(vec2 p) {
	return SIN((p.x + p.y)*2.) +SIN((p.x + p.y)*4.)*.5+SIN((p.x + p.y)*1.)*4.;
}
...
vec4 materialColorForPixel(vec2 texCoord)
{
	...
	p.x += noise(p*1.5 + vec2(0, mat_automovespeed*.5))*.2*mat_distort;
	p.x += noise(p*5. + vec2(0, mat_automovespeed*.5))*.04*mat_distort;
	...
	if(mat_oneshot) {
		value *= 1.-smoothstep(.5, 1., mat_move_position);
	}
