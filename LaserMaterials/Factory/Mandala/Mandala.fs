/*{
	...
	"DESCRIPTION": "The mandala is parametric curve with animations on it",
	"TAGS": "laser",
	"VSN": "1.0",
	"INPUTS": [
		...
		{"LABEL": "Global/Line Length", "NAME": "mat_length", "TYPE": "float", "MIN": 0., "MAX": 4., "DEFAULT": 1.5, "DESCRIPTION":"To adjust the length of the lines"},  
		
		{"LABEL": "Animation/Adjustement", "NAME": "mat_insideShift", "TYPE": "point2D", "DEFAULT": [1.25,-0.825],"MIN":[-1.5,-1.5],"MAX":[1.5,1.5], "DESCRIPTION":"The line is controled by a parametric function, you can adjust the parameters of the function. x axe : frequency, y axe : y amplitude"}, 
		{"LABEL": "Animation/Bounce Length", "NAME": "mat_lengthActivated", "TYPE": "bool", "DEFAULT": true, "FLAGS": "button", "DESCRIPTION":"To activate the strobe"},
		{"LABEL": "Animation/Length Speed", "NAME": "mat_offsetGeneral", "TYPE": "float", "DEFAULT": 0.15, "MIN": 0, "MAX": 1., "DESCRIPTION":"The animation is making the lenght oscillates betwee 0 and the -Length- variable, here adjust the speed of the animation"},
		{"LABEL": "Animation/Bounce Size", "NAME": "mat_bounceActivated", "TYPE": "bool", "DEFAULT": true, "FLAGS": "button", "DESCRIPTION":"To activate the strobe"},
		{"LABEL": "Animation/Bounce Speed", "NAME": "mat_speedAnim", "TYPE": "float", "MIN": 0., "MAX": 1., "DEFAULT":0.16, "DESCRIPTION":"The bounce animation is multiplying the size of the visual with an oscillator, adjust the speed here"},  
		{"LABEL": "Animation/Auto Rotate", "NAME": "mat_rotateActivated", "TYPE": "bool", "DEFAULT": true, "FLAGS": "button", "DESCRIPTION":"To activate the strobe"},
		{"LABEL": "Animation/Speed Rotate", "NAME": "mat_speedRotate", "TYPE": "float", "DEFAULT": 0.7, "MIN": -1.0, "MAX": 1.0 , "DESCRIPTION":"You cand adjust here the speed of an auto rotate animation"},
		
		{"LABEL": "Color/Mod", "NAME": "mat_button_grid", "TYPE": "long", "DEFAULT":"Sin 2", "VALUES": ["Main","Grad","Osc","Sin 2","Sin 4","Sin 8","Pulse","Pulse2","PuPulse"], "DESCRIPTION":"You can choose your color Mod. Main : Main Color, Grad : Gradient color between main and secondary color, Osc : Oscillate between main and secondary color in time, Sin : Oscillate between main and secondary color with a sin, Pulse : impulsion of color in the line, PuPulse : the same as pulse with frequency modulation", "FLAGS": "button_grid" },
	...
	"GENERATORS": [
		...
		{"NAME": "mat_timeRotate", "TYPE": "time_base", "PARAMS": {"speed": "mat_speedRotate","reverse": "mat_animationReverse","speed_curve": 1,"link_speed_to_global_bpm":true}},	
...
mat3 makeTransformMatrix(float normalizedCellId)
{
	...
	float base_rotation = (mat_rotateActivated?-mat_timeRotate*100:0.)+45*mat_button_angle;
	// Center
...
{	
	...
	int pointsPerShape = finalPointCount/1;
	...
	float normalizedPosInShapeMax = (mat_lengthActivated?2.*(osc_length)*mat_length:2.*mat_length) * (normalizedPosInShape - 0.5);
	...
	float autoShift = mat_bounceActivated?sin(PI*mat_timeShift*0.9):1.;
	float autoShift2 = mat_bounceActivated?sin(PI*(mat_timeShift)*0.9):1.;
	...
	if(mat_button_grid==0){
		...
		color = vec4(mix(vec4(mat_mainColor.rgb,1.),vec4(mat_secondaryColor.rgb,mat_alpha),sin(abs(2.*PI*normalizedPosInShapeMax+sin(mat_timeOffset)*20.))*0.5+0.5));
	}
