/*{
	...
	"DESCRIPTION": "Bouncing lines pattern, choose your preset",
	"TAGS": "laser",
	"VSN": "1.0",
	"INPUTS": [
		...
		{"LABEL": "Global/Global Size", "NAME": "mat_sizeGlobal", "TYPE": "float", "MIN": 0., "MAX": 1., "DEFAULT": 1., "DESCRIPTION":"Global size"}, 
		...
		{"LABEL": "Shape Manager/Shape Rotate", "NAME": "mat_autorotateoffset", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 360.0, "DESCRIPTION":"To adjust the offset angle according to the line number"},

		{"LABEL": "Animation/Preset", "NAME": "mat_animMod", "TYPE": "long", "DEFAULT":"1", "VALUES": ["1","2","3","4","5","6","7","8","9"], "FLAGS": "button_grid", "DESCRIPTION":"Each Mod has his own preset of variables, choose the one you want"},
		{"LABEL": "Animation/Global Speed", "NAME": "mat_speedGlobal", "TYPE": "float", "MIN": 0.0, "MAX": 5., "DEFAULT": 0.7, "DESCRIPTION":"To adjust the speed of all the animation"},
	...
	"GENERATORS": [
		...
		{"NAME": "mat_timeSize", "TYPE": "time_base", "PARAMS": {"speed": "mat_speedGlobal","speed_curve": 3,"link_speed_to_global_bpm":true}},
		{"NAME": "mat_rot_position","TYPE": "time_base","PARAMS": {"speed": "mat_speedGlobal", "speed_curve":3, "link_speed_to_global_bpm":true, "max_value":10000 }},
		{"NAME": "mat_multiplier","TYPE": "multiplier","PARAMS": {"value1": "mat_value", "value2": 2, "value3": 1, "value4": 1}}
	],
	"RENDER_SETTINGS": {
	   "POINT_COUNT": 500,
	},
...
mat3 makeTransformMatrix(float normalizedCellId,float timeSideBounce,float mat_ampSideBounce,int shapeNumber)
{
	float base_rotation = int(mat_sideBounceActivated)*20*(!bool(mat_speedSideBounceMulti)?0.5:mat_speedSideBounceMulti)*mat_ampSideBounce*sin(2*PI*timeSideBounce)+45*mat_button_angle+mat_offsetAngle+mat_autorotateoffset*shapeNumber;
	// Center
...
{
	userData.x = mat_shapeAmount+mat_button_lineMod;
	...
	float timeBounce = mat_timeBounce * 0.2 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
	float ampBounce = 0.8;
	float offsetBounce = 0.8;
	float phaseBounce = 0.0;

	float timeSize = mat_timeSize * 0.1 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
	float offsetLineSize = 0.;
	float phaseLineSize = 1;
	float delayLineSize = 0.5;//int(mat_delayLineSize);

	float timeSideBounce = mat_timeSideBounce * 0.75 *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
	float ampSideBounce = 1.2;

	if(mat_animMod==1){
		timeBounce = mat_timeBounce * 0.1 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti)*2.;
		ampBounce = 0.85;
		offsetBounce = 0.0;
		phaseBounce = 0.25;

		timeSize = mat_timeSize * 0.1 * (mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  *2.;
		offsetLineSize = 0.;
		phaseLineSize = 0.25;
		delayLineSize = 0.;//int(mat_delayLineSize);

		timeSideBounce = mat_timeSideBounce * 0.1 *2.*(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2;
	} else if(mat_animMod==2){
		timeBounce = mat_timeBounce * 0.3 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 0.85;
		offsetBounce = 0.0;
		phaseBounce = 0.;
		...
		timeSize = mat_timeSize * 0.25 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 0.2;
		phaseLineSize = 0.5;
		delayLineSize = 0.;//int(mat_delayLineSize);

		timeSideBounce = mat_timeSideBounce * 0.125  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2;
	} else if(mat_animMod==4){
		timeBounce = mat_timeBounce * (0.35/2.)   * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti)*2.;
		ampBounce = 0.7;
		offsetBounce = 0.8;
		phaseBounce = 0.;

		timeSize = mat_timeSize * 0.05 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  *2.;
		offsetLineSize = 0.2;
		phaseLineSize = 1;
		...
		timeSideBounce = mat_timeSideBounce *0.25  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 1.;
		...
		timeSize = mat_timeSize * 6. *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 1.5;
		phaseLineSize = 1;
		delayLineSize = 0.;//int(mat_delayLineSize);
	...
	mat3 transformMatrix = makeTransformMatrix(normalizedShapeId,timeSideBounce,ampSideBounce,shapeNumber);
	...
	if(FRAMEINDEX > 0) {
		if (mat_shapeAmount+mat_button_lineMod == lastFrameShapeAmount) {
