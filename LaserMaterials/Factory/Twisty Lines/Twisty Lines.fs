/*{
	...
	"DESCRIPTION": "Rotating lines pattern, choose your preset",
	"TAGS": "laser",
	"VSN": "1.0",
	"INPUTS": [
		...
		{"LABEL": "Shape Manager/Line Mod", "NAME": "mat_button_lineMod", "TYPE": "long", "DEFAULT":"Mirror", "VALUES": ["Center","Mirror","Clock"], "FLAGS": "button_grid", "DESCRIPTION":"To choose de line configuration mod, Center : Add an extra line at the center, Mirror : No extra line in the center, Clock : same center, different angle"},
		{"LABEL": "Shape Manager/Line Size", "NAME": "mat_size", "TYPE": "float", "MIN": 0., "MAX": 2., "DEFAULT": 1., "DESCRIPTION":"To adjust the size of the lines"}, 
		{"LABEL": "Shape Manager/Shape Rotate", "NAME": "mat_shapeRotate", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 90.0, "DESCRIPTION":"To adjust the offset angle according to the line number"},

		{"LABEL": "Animation/Preset", "NAME": "mat_animMod", "TYPE": "long", "DEFAULT":"1", "VALUES": ["1","2","3","4","5","6","7","8","9"], "FLAGS": "button_grid", "DESCRIPTION":"Each Mod has his own set of variables, choose the one you want"},
		{"LABEL": "Animation/Global Speed", "NAME": "mat_speedGlobal", "TYPE": "float", "MIN": 0.0, "MAX": 5.0, "DEFAULT": 0.7, "DESCRIPTION":"To adjust the speed of all the animation"},
		
		{"LABEL": "Rotation Bounce/Active", "NAME": "mat_bounceActivated", "TYPE": "bool", "DEFAULT": true, "FLAGS": "button", "DESCRIPTION":"To activate the rotation bounce. The rotation bounce is an oscillator mapped on the rotation offset according to the shape number"},
		{"LABEL": "Rotation Bounce/Speed", "NAME": "mat_speedBounceMulti", "TYPE": "long", "DEFAULT":"x1", "VALUES": ["x0.5","x1","x2"], "FLAGS": "button_grid", "DESCRIPTION":"To multiply the bounce speed by 0.5, 1 or 2"},
		{"LABEL": "Rotation Bounce/Amplitude", "NAME": "mat_ampBounceMulti", "TYPE": "long", "DEFAULT":"x1", "VALUES": ["x0.5","x1","x2"], "FLAGS": "button_grid", "FLAGS": "button_grid", "DESCRIPTION":"To multiply the side bounce amplitude by 0.5, 1 or 2"},
		
		{"LABEL": "Side Bounce/Active", "NAME": "mat_sideBounceActivated", "TYPE": "bool", "DEFAULT": true, "FLAGS": "button", "DESCRIPTION":"To activate the side bounce animation"},
		{"LABEL": "Side Bounce/Speed", "NAME": "mat_speedSideBounceMulti", "TYPE": "long", "DEFAULT":"x1", "VALUES": ["x0.5","x1","x2"], "FLAGS": "button_grid", "FLAGS": "button_grid", "DESCRIPTION":"To multiply the side bounce speed by 0.5, 1 or 2"},
		{"LABEL": "Side Bounce/Amplitude", "NAME": "mat_ampSideBounceMulti", "TYPE": "long", "DEFAULT":"x1", "VALUES": ["x0.5","x1","x2"], "FLAGS": "button_grid", "FLAGS": "button_grid", "DESCRIPTION":"To multiply the side bounce amplitude by 0.5, 1 or 2"},
		
		{"LABEL": "Auto Rotate/Active", "NAME": "mat_autorotateactive", "TYPE": "bool", "DEFAULT": true, "FLAGS": "button", "DESCRIPTION":"To activate the auto rotate animation"},
		{"LABEL": "Auto Rotate/Speed", "NAME": "mat_speedRotateMulti", "TYPE": "long", "DEFAULT":"x1", "VALUES": ["x0.5","x1","x2"], "FLAGS": "button_grid", "DESCRIPTION":"To multiply the auto rotate speed by 0.5, 1 or 2"},
		{"LABEL": "Auto Rotate/Reverse", "NAME": "mat_autorotatereverse", "TYPE": "bool", "DEFAULT": false, "FLAGS": "button", "DESCRIPTION":"To reverse the spin"},
		...
		{"LABEL": "Color/Secondary", "NAME": "mat_secondaryColor", "TYPE": "color", "DEFAULT": [ 0.0, 0.0, 1.0, 1.0 ] ,"FLAGS" :"no_alpha", "DESCRIPTION":"To choose the secondary color"},
	...
	"GENERATORS": [
		...
		{"NAME": "mat_timeBounce", "TYPE": "time_base", "PARAMS": {"speed": "mat_speedGlobal","speed_curve": 3,"link_speed_to_global_bpm":true}},
		{"NAME": "mat_timeOscColor", "TYPE": "time_base", "PARAMS": {"speed": "mat_speedOscColor","speed_curve": 3,"link_speed_to_global_bpm":true}},
		{"NAME": "mat_timeSideBounce", "TYPE": "time_base", "PARAMS": {"speed": "mat_speedGlobal","speed_curve": 3,"link_speed_to_global_bpm":true}},
		{"NAME": "mat_timeSize", "TYPE": "time_base", "PARAMS": {"speed": "mat_speedGlobal","speed_curve": 1,"link_speed_to_global_bpm":true}},
		{"NAME": "mat_rot_position","TYPE": "time_base","PARAMS": {"speed": "mat_speedGlobal","reverse": "mat_autorotatereverse","strob": "mat_autorotatestrob", "speed_curve":3, "link_speed_to_global_bpm":true, "max_value":10000 }},
	],
	"RENDER_SETTINGS": {
	   "POINT_COUNT": 500,
	},
...
{
	float base_rotation = 360.*normalizedCellId*autorotateoffset+int(mat_sideBounceActivated)*20*(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti)*mat_ampSideBounce*sin(2*PI*timeSideBounce)+45*mat_button_angle+mat_offsetAngle+mat_shapeRotate*shapeNumber;
	// Center
	...
	if (mat_autorotateactive || (base_rotation!=0)) {
		...
		angle += fract(0.5 + (timeAutorotate)) * 2*PI;
		}
...
{
	userData.x = mat_shapeAmount+int(mat_button_lineMod)+int(mat_animMod);
	//userData.y = mat_button_lineMod;
	...
	float timeBounce = mat_timeBounce * 0.4 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
	float ampBounce = 0.5 * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
	float offsetBounce = 0.;
	float phaseBounce = 0.0;

	float autorotateoffset = (mat_bounceActivated?(ampBounce*sin(2*PI*(timeBounce))+offsetBounce):0)+0.5;

	float timeAutorotate = mat_rot_position * 1. * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

	float timeSideBounce = mat_timeSideBounce * 1. * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
	float ampSideBounce = 2;

	timeBounce = mat_timeBounce * 1.   * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
	ampBounce = 0.10;
	offsetBounce = 0.;
	phaseBounce = 0.;

	timeAutorotate = mat_rot_position * 0.3 * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

	timeSideBounce = mat_timeSideBounce *0.1  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
	ampSideBounce = 2.;

	if(mat_animMod==1){
		timeBounce = mat_timeBounce * 0.25 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 0.5 * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 0.;
		phaseBounce = 0.0;

		autorotateoffset = (mat_bounceActivated?(ampBounce*sin(2*PI*(timeBounce))+offsetBounce):0)+0.5;

		timeAutorotate = mat_rot_position * 0.25 * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSideBounce = mat_timeSideBounce * 0.25 * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
		ampSideBounce = 2;
	}
	
	else if(mat_animMod==2){
		timeBounce = mat_timeBounce * 0.05 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 3. * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 0.;
		phaseBounce = 0.0;
		...
		timeSideBounce = mat_timeSideBounce * 0.5 * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
		ampSideBounce = 0.5;
	}

	else if(mat_animMod==3){
		timeBounce = mat_timeBounce * 0.6 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce =0.16 * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 0.5;
		phaseBounce = 0.0;
		...
		timeSideBounce = mat_timeSideBounce * 0.3 * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
		ampSideBounce = 0.5;
	}

	else if(mat_animMod==4){
		timeBounce = mat_timeBounce * 0.1* (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 0.5 * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 0.;
		phaseBounce = 0.0;
	...
	else if(mat_animMod==5){
		timeBounce = mat_timeBounce * 0.6 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce =0.08 * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 1.;
		phaseBounce = 0.0;
	...
	else if(mat_animMod==6){
		timeBounce = mat_timeBounce * 0.25 * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 5. * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 0.;
		phaseBounce = 0.0;
		...
		timeAutorotate = mat_rot_position * 0. * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSideBounce = mat_timeSideBounce * 0.2 * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
		ampSideBounce = 1.;
	}

	else if(mat_animMod==8){
		timeBounce = mat_timeBounce * 1. * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 5. * (mat_ampBounceMulti==0?0.5:mat_ampBounceMulti);
		offsetBounce = 0.;
		phaseBounce = 0.0;
		...
		timeAutorotate = mat_rot_position * 10. * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSideBounce = mat_timeSideBounce * 2. * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
		ampSideBounce = 1.;
	}
	...
	if(FRAMEINDEX > 0) {
		if (mat_shapeAmount+int(mat_button_lineMod)+int(mat_animMod) == lastFrameShapeAmount) {
