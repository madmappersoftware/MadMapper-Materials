/*{
	...
	"INPUTS": [ 
		...
		{"LABEL": "Animation/Length", "NAME": "mat_length", "TYPE": "float", "MIN": 0., "MAX": 1., "DEFAULT": 1., "DESCRIPTION":"To adjust the length of the lines" }, 
		...
		{"LABEL": "Animation/Mirror", "NAME": "mat_twoLinesButton", "TYPE": "bool", "DEFAULT": true,"FLAGS":"button", "DESCRIPTION":"To add one reversed line"}, 
		{"LABEL": "Animation/Animation mod", "NAME": "mat_insideShift", "TYPE": "point2D", "DEFAULT": [1,-1],"MIN":[-1,-1],"MAX":[1,1], "DESCRIPTION":"The lines are controled by a parametric function, you can adjust the parameters of the function. x axe : frequency, y axe : y amplitude"}, 
		{"LABEL": "Animation/Crush", "NAME": "mat_crush", "TYPE": "float", "DEFAULT": 0.5, "MIN":0., "MAX": 1.0, "DESCRIPTION":"To adjust the amplitude of the shape" },
		
		{"LABEL": "Color/Mod", "NAME": "mat_button_grid", "TYPE": "long", "DEFAULT":"Sin 2", "VALUES": ["Main","Grad","Osc","Sin 2","Sin 4","Sin 8","Flow","Flow 2","Loop"], "DESCRIPTION":"You can choose your color Mod. Main : Main Color, Grad : Gradient color between main and secondary color, Osc : Oscillate between main and secondary color in time, Sin : Oscillate between main and secondary color with a sin, Flow : Color oscillation in space, Loop : Pulse of colors in loop", "FLAGS": "button_grid" },
	...
	"GENERATORS": [
		...
		{"NAME": "mat_timeOffset", "TYPE": "time_base", "PARAMS": {"speed": "mat_offsetMultiplied","speed_curve": 1,"link_speed_to_global_bpm":false}},
		{"NAME": "mat_offsetMultiplied","TYPE": "multiplier","PARAMS": {"value1": "mat_speedOffset", "value2":3, "value3": 1, "value4": 1}},
		{"NAME": "osc_offset", "TYPE": "animator", "PARAMS": {"speed": "mat_offsetMultiplied","reverse": "mat_animationReverse","speed_curve": 2, "shape":"Smooth", "link_speed_to_global_bpm":true}},
	],
	"RENDER_SETTINGS": {
	   "POINT_COUNT": 500,
	},
...
{
	int finalPointCount = pointsForPrecision[mat_precision-1]/(mat_twoLinesButton?1:2);//4 + (mat_precision/10)*(500-4));
	...
	float normalizedPosMaxLeft = (normalizedPosInShapeMax*sin(1.*(-mat_timeOffset-0.5)))*2-1;

	float normalizedPos = float(pointNumber)/(finalPointCount-1);
	float offset = 10;

	float osc = cos(0.1*PI*(normalizedPosInShapeMax-mat_timeOffset))*0.5+0.5;
	float oscOffseted = cos(0.1*PI*(normalizedPosInShapeMax-mat_timeOffset+10.))*0.5+0.5;
	...
	float shapeNumberVar = 2*(shapeNumber-0.5);

	pos = vec2(
		x * (0.5*sin(1*PI*osc_offset)+0.5) * shapeNumberVar*1.
		,y * (0.5*sin(1*(PI+1)+0.5)*osc_offset)*mat_crush*2.
	);
	...
	if(mat_button_grid==0){
		...
		color = vec4(mix(vec4(mat_mainColor.rgb,1.),vec4(mat_secondaryColor.rgb,mat_alpha),1-fract((!mat_twoLinesButton?1.:2.)*normalizedPos)));
	}
