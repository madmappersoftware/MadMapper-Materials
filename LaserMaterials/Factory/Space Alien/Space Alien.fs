/*{
	...
	"DESCRIPTION": "Alien glitchy looking visual using line pattern with color gradient",
	"TAGS": "laser",
	"VSN": "1.0",
	"INPUTS": [
		...
		{"LABEL": "Auto Bounce/Amplitude", "NAME": "mat_ampBounce", "TYPE": "float", "MIN": 0.0, "MAX": 2.0, "DEFAULT": 0.75, "DESCRIPTION":"To adjust the amplitude of the bounce animation"},
...
void laserMaterialFunc(int pointNumber, int pointCount, out vec2 pos, out vec4 color, out int shapeNumber, out vec4 userData)
{
	...
	if(!bool(mat_buttonGlitch)){
		bounce = pow(abs(tan(2*PI*normalizedPos)),(mat_bounceActivated?(mat_ampBounce*sin(2*PI*(mat_timeBounce+mat_phaseBounce))+mat_offsetBounce)*10:1.));
		bounce2 = pow(abs(tan(2*PI*normalizedPos)),(mat_bounceActivated?(mat_ampBounce*sin(2*bounce*PI*(mat_timeBounce+mat_phaseBounce))+mat_offsetBounce)*10:1.));
	}
	else{
		bounce = pow(abs(tan(2*PI*normalizedPos)),(mat_bounceActivated?(mat_ampBounce*sin(2*PI*(mat_timeBounce+mat_phaseBounce))+mat_offsetBounce)*10:1.));
		bounce2 = pow(abs(tan(2*PI)),(mat_bounceActivated?(mat_ampBounce*sin(2*bounce*PI*(mat_timeBounce+mat_phaseBounce))+mat_offsetBounce)*10:1.));
	}
	color = vec4(mix(vec4(mat_mainColor.rgb,1.),vec4(0.),(cos(mat_shapeAmount*2*PI*normalizedPos)+1)));
	if(mat_button_grid==0){
		...
		color *= vec4(mix(vec4(mat_mainColor.rgb,1.),vec4(mat_secondaryColor.rgb,mat_alpha),(sin(mat_timeBounce*2*PI)+1)/2));
	}

	color[3]=color[3]*bounce2;
