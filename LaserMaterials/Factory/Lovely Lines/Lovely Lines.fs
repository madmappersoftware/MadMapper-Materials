/*{
	...
	"DESCRIPTION": "Dancing lines pattern, choose your preset",
	"TAGS": "laser",
	"VSN": "1.0",
	"INPUTS": [
		...
		{"LABEL": "Bounce/Speed", "NAME": "mat_speedBounceMulti", "TYPE": "long", "DEFAULT":"x1", "VALUES": ["x0.5","x1","x2"], "FLAGS": "button_grid", "DESCRIPTION":"To multiply the bounce speed by 0.5, 1 or 2"},
...
{
	float base_rotation = int(mat_sideBounceActivated)*20*(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti)*mat_ampSideBounce*sin(2*PI*timeSideBounce)+45*mat_button_angle+mat_offsetAngle+shapeNumber*mat_offsetAngleShape;
	// Center
...
void laserMaterialFunc(int pointNumber, int pointCount, out vec2 pos, out vec4 color, out int shapeNumber, out vec4 userData)
{
	...
	float timeAutorotate = mat_rot_position * 0.1 * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

	float timeSize = mat_timeSize * 0.2 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
	float offsetLineSize = 0.2;
	float phaseLineSize = 1;
	float delayLineSize = 0.5;//int(mat_delayLineSize);

	float timeSideBounce = mat_timeSideBounce * 0.2   * (mat_speedSideBounceMulti==0?0.5:mat_speedSideBounceMulti);
	...
	if(mat_animMod==1){
		...
		timeAutorotate = mat_rot_position * 0.2 * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSize = mat_timeSize * 0.1*(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 0.;
		phaseLineSize = 0.25;
	...
	if(mat_animMod==2){
		timeBounce = mat_timeBounce * 0.15   * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 0.85;
		offsetBounce = 0.0;
		phaseBounce = 0.;
		...
		timeSideBounce = mat_timeSideBounce * 0.2  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2;
	}
	...
	if(mat_animMod==3){
		...
		timeSize = mat_timeSize * 0.025 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 0.2;
		phaseLineSize = 1;
		delayLineSize = 0.;//int(mat_delayLineSize);

		timeSideBounce = mat_timeSideBounce * (0.33333/2.)  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2;
	}

	if(mat_animMod==4){
		timeBounce = mat_timeBounce * (0.35/2.)   * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 0.7;
		offsetBounce = 0.8;
		phaseBounce = 0.;

		timeAutorotate = mat_rot_position * (0.35/2.) * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSize = mat_timeSize * 0.05 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 0.2;
		phaseLineSize = 1;
		delayLineSize = 0.5;//int(mat_delayLineSize);

		timeSideBounce = mat_timeSideBounce * 0.15  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2;
	}

	if(mat_animMod==5){
		timeBounce = mat_timeBounce * 0.5   * (mat_speedBounceMulti==0?0.5:mat_speedBounceMulti);
		ampBounce = 0.5;
		offsetBounce = 1.;
		phaseBounce = 0.;
		...
		timeSideBounce = mat_timeSideBounce *0.5  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 1.;
	}
	...
	if(mat_animMod==6){
		...
		timeAutorotate = mat_rot_position * 0.15 * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSize = mat_timeSize * 0.1 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 0.2;
		phaseLineSize = 1;
		delayLineSize = 0.6;//int(mat_delayLineSize);

		timeSideBounce = mat_timeSideBounce *0.3  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2.;
		autorotateoffset = mat_autorotateoffset + 0.5;
	}
	...
	if(mat_animMod==7){
		...
		offsetLineSize = 0.8;
		phaseLineSize = 1;
		delayLineSize = 0.12;//int(mat_delayLineSize);

		timeSideBounce = mat_timeSideBounce *0.05  *(mat_ampSideBounceMulti==0?0.5:mat_ampSideBounceMulti);
		ampSideBounce = 2.;
	}
	...
	if(mat_animMod==8){
		...
		timeAutorotate = mat_rot_position * 0.1 * (mat_speedRotateMulti==0?0.5:mat_speedRotateMulti)  ;

		timeSize = mat_timeSize * 0.5 *(mat_speedLineSizeMulti==0?0.5:mat_speedLineSizeMulti)  ;
		offsetLineSize = 0.8;
		phaseLineSize = 1;
		delayLineSize = 0.;//int(mat_delayLineSize);
