/*{
  "INPUTS": [
    ...
    { "LABEL": "Auto Light/Size", "NAME": "mat_autolightsize", "TYPE": "float", "DEFAULT": 1, "MIN": 0.0, "MAX": 2.0 },
    ...
    { "LABEL": "Auto Size/Active", "NAME": "mat_autosizeactive", "TYPE": "bool", "DEFAULT": false, "FLAGS": "button" },
    { "LABEL": "Auto Size/Size", "NAME": "mat_autosizesize", "TYPE": "float", "DEFAULT": 1, "MIN": 0.0, "MAX": 1.0 },
    { "LABEL": "Auto Size/Speed", "NAME": "mat_autosizespeed", "TYPE": "float", "DEFAULT": 3.0, "MIN": 0.0, "MAX": 3.0 },
    { "LABEL": "Auto Size/Strob", "NAME": "mat_autosizestrob", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 1.0 },
    { "LABEL": "Auto Size/Shape", "NAME": "mat_autosizeshape", "TYPE": "long", "VALUES": ["Smooth","In","Linear","Noise"], "DEFAULT": "Smooth" },
    { "LABEL": "Auto Size/Offset", "NAME": "mat_autosizeoffset", "TYPE": "float", "DEFAULT": 1.0, "MIN": 0.0, "MAX": 1.0 },
  ...
  "GENERATORS": [
    ...
    {
      "NAME": "mat_size_position",
      "TYPE": "time_base",
      "PARAMS": {"speed": "mat_autosizespeed","strob": "mat_autosizestrob", "bpm_sync": "mat_bpmsync", "speed_curve":2, "link_speed_to_global_bpm":true, "max_value":10000 }
    },
    {
...
float getLightValue(float normalizedCellId)
{
    ...
    if (mat_autolightactive) {
      ...
      if (mat_autolightshape == 0) {
        light -= pow(mat_autolightsize,3) * (1+sin((mat_light_position+normalizedCellId*mat_autolightoffset)*2*PI))/2;
      } else if (mat_autolightshape == 1) {
        light -= pow(mat_autolightsize,3) * (mod((mat_light_position+normalizedCellId*mat_autolightoffset),1));
      } else {
		float size = mix(0,1,mat_autolightsize/2);
        light -= pow(mat_autolightsize,3) * step(1-size,mod((mat_light_position+normalizedCellId*mat_autolightoffset),1));
      }
    }
    return light;
}

float getSizeValue(float normalizedCellId)
{
    // Size
    float size = 1;
    if (mat_autosizeactive) {
      // "Smooth","In","Linear","Cut","Noise"
      if (mat_autosizeshape == 0) {
        size -= mat_autosizesize * (1+sin((-mat_size_position+normalizedCellId*mat_autosizeoffset)*2*PI))/2;
      } else if (mat_autosizeshape == 1) {
        size -= mat_autosizesize * (mod((-mat_size_position+normalizedCellId*mat_autosizeoffset),1));
      } else if (mat_autosizeshape == 2) {
        size -= mat_autosizesize * abs(mod((-mat_size_position+normalizedCellId*mat_autosizeoffset)*2+1,2));
      } else {
        size -= mat_autosizesize * noise(vec2((mat_size_position+normalizedCellId*mat_autosizeoffset*99.5),0));
      }
    }
    return size;
}
