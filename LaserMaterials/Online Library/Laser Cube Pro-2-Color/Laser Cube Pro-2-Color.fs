/*{
	...
	"DESCRIPTION": "A laser shader that draws an animated 3D cube with a bi-color gradient, reversible rotation, and animated colors",
	...
	"INPUTS": [
		...
		{"LABEL": "Color Animation Speed", "NAME": "color_speed", "TYPE": "float", "MIN": 0.0, "MAX": 1.0, "DEFAULT": 0.2 }
	],
	"GENERATORS": [
		{"NAME": "anim_time", "TYPE": "time_base", "PARAMS": {"speed": "rotation_speed", "speed_curve": 2, "link_speed_to_global_bpm": false }},
		{"NAME": "color_time", "TYPE": "time_base", "PARAMS": {"speed": "color_speed", "speed_curve": 2, "link_speed_to_global_bpm": false }}
	]
}*/
...
void laserMaterialFunc(int pointNumber, int pointCount, out vec2 pos, out vec4 color, out int shapeNumber, out vec4 userData) {
	...
	// Animate colors using time-based factor
	float colorAnimFactor = 0.5 + 0.5 * sin(color_time * 2.0 * pi); // Oscillates between 0 and 1

	// Assign a bi-color gradient with animated color blending
	float gradientFactor = float(edgeIndex) / 12.0; // 0 to 1 across all 12 edges
	vec4 animatedColor1 = mix(color1, color2, colorAnimFactor);
	vec4 animatedColor2 = mix(color2, color1, colorAnimFactor);
	color = mix(animatedColor1, animatedColor2, gradientFactor);
