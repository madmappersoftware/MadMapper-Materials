/*{
	...
	"DESCRIPTION": "A laser shader that draws an animated 3D cube with a 4-color gradient, reversible rotation, and animated colors",
...
void laserMaterialFunc(int pointNumber, int pointCount, out vec2 pos, out vec4 color, out int shapeNumber, out vec4 userData) {
	...
	// Assign a 4-color gradient with animated blending
	float gradientFactor = float(edgeIndex) / 12.0; // 0 to 1 across all 12 edges
	vec4 blendedColor1 = mix(color1, color2, colorAnimFactor);
	vec4 blendedColor2 = mix(color3, color4, colorAnimFactor);
	color = mix(blendedColor1, blendedColor2, gradientFactor);
